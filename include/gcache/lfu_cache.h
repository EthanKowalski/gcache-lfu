...
class LFUCache {
   ...
   * lifecycle of LFUCache; if a LFU replacement happens, a reused handle will
   ...
   * with the usage pattern of LFU cache. In a typically use case, the key field
   ...
   * pointer; when a LFU replacement happens, the same physical page will be
 ...
 public:
  ...
  // For each item in the LFU list, call fn(key, handle) in LFU order
  template <typename Fn>
  void for_each_lfu(Fn&& fn) const;

  // For each item in the LFU list, call fn(key, handle) in MFU order
  template <typename Fn>
  void for_each_mfu(Fn&& fn) const;
  ...
  template <typename Fn>
  void for_each_until_lfu(Fn&& fn) const;
  template <typename Fn>
  void for_each_until_mfu(Fn&& fn) const;

  // Set pin to be true to pin the returned node so it won't be recycled by
  // LFU; a pinned node must be unpinned later by calling release().
  ...
  // Search for a node; return nullptr if not exist. This op will refresh LFU.
   ...
   * to flow among the lfu list, the in-use list, and the free list.
  ...
  // Erase the non-null handle from the lfu list; return whether erasing
  // succeeds (fail if not in lfu). The erased handle will not be reused by
  // `insert`, and the caller should save the value if necessary.
  bool erase(Handle_t handle);

  // Install a handle into the lfu list. This action will not use free list or
  // lfu list but allocate additional space or reused space from erased
 ...
 private:
  ...
  // lfu_ instead; if still not available, return nullptr.
  Handle_t preempt();

  // Assign a new node to this LFUCache; will be put into free list (duel with
  // `preempt`).
  ...
  // Similar to insert but 1) the targeted node must be in LFU list and this
  // function never pins it, 2) return `successor`: the node with the same order
  // as the returned node after LFU operations (nullptr if newly inserted).
 ...
 private:
  ...
  // Helper function for lookup: 1) pin the node if asked; 2) refresh LFU if in
  // the LFU list
  ...
  // after LFU (usually it's e->next)
  Node_t* lfu_refresh(Node_t* e);
  ...
  // must either present in lfu_ or in_use_
  ...
  // lfu.prev is the newest entry, lfu.next is the oldest entry.
  // Entries have refs==1.
  Node_t lfu_;
 ...
 public:  // for debugging
  ...
  friend std::ostream& operator<<(std::ostream& os, const LFUCache& c) {
    return c.print(os);
  }
};
...
inline LFUCache<Key_t, Value_t, Hash>::~LFUCache() {
  ...
  if (pool_) {
    ...
    // for (const Node_t* e = lfu_.next; e != &lfu_; e = e->next)
    //   assert(e->refs == 1);  // Invariant of lfu_ list.
    delete[] pool_;
    delete table_;
  }
...
template <typename Key_t, typename Value_t, typename Hash>
inline void LFUCache<Key_t, Value_t, Hash>::init(size_t capacity) {
...
template <typename Key_t, typename Value_t, typename Hash>
template <typename Fn>
inline void LFUCache<Key_t, Value_t, Hash>::init(size_t capacity, Fn&& fn) {
  init(capacity);
...
template <typename Key_t, typename Value_t, typename Hash>
template <typename Fn>
inline void LFUCache<Key_t, Value_t, Hash>::for_each(Fn&& fn) const {
  for_each_lfu(fn);
  for_each_in_use(fn);
}
...
template <typename Key_t, typename Value_t, typename Hash>
template <typename Fn>
inline void LFUCache<Key_t, Value_t, Hash>::for_each_lfu(Fn&& fn) const {
  for (auto h = lfu_.next; h != &lfu_; h = h->next) fn(h);
}

template <typename Key_t, typename Value_t, typename Hash>
template <typename Fn>
inline void LFUCache<Key_t, Value_t, Hash>::for_each_mfu(Fn&& fn) const {
  for (auto h = lfu_.prev; h != &lfu_; h = h->prev) fn(h);
}

template <typename Key_t, typename Value_t, typename Hash>
template <typename Fn>
inline void LFUCache<Key_t, Value_t, Hash>::for_each_in_use(Fn&& fn) const {
  for (auto h = in_use_.next; h != &in_use_; h = h->next) fn(h);
}

template <typename Key_t, typename Value_t, typename Hash>
template <typename Fn>
inline void LFUCache<Key_t, Value_t, Hash>::for_each_until_lfu(Fn&& fn) const {
  for (auto h = lfu_.next; h != &lfu_; h = h->next) {
    if (!fn(h)) break;
  }
}
...
template <typename Key_t, typename Value_t, typename Hash>
template <typename Fn>
inline void LFUCache<Key_t, Value_t, Hash>::for_each_until_mfu(Fn&& fn) const {
  for (auto h = lfu_.prev; h != &lfu_; h = h->prev)
    if (!fn(h)) break;
}

template <typename Key_t, typename Value_t, typename Hash>
inline void LFUCache<Key_t, Value_t, Hash>::init_from(
...
template <typename Key_t, typename Value_t, typename Hash>
inline typename LFUCache<Key_t, Value_t, Hash>::Handle_t
LFUCache<Key_t, Value_t, Hash>::insert(Key_t key, bool pin,
                                       bool hint_nonexist) {
...
LFUCache<Key_t, Value_t, Hash>::insert_impl(Key_t key, uint32_t hash, bool pin,
                                            bool hint_nonexist) {
  ...
  if (!hint_nonexist) {  // if not sure whether the key exists, do lookup
    e = lookup_impl(key, hash, pin);  // lookup_impl will do LFU refresh
    if (e) return e;
  } else {
...
template <typename Key_t, typename Value_t, typename Hash>
inline typename LFUCache<Key_t, Value_t, Hash>::Handle_t
LFUCache<Key_t, Value_t, Hash>::lookup(Key_t key, bool pin) {
  return lookup_impl(key, Hash{}(key), pin);
}

template <typename Key_t, typename Value_t, typename Hash>
inline typename LFUCache<Key_t, Value_t, Hash>::Node_t*
LFUCache<Key_t, Value_t, Hash>::lookup_impl(Key_t key, uint32_t hash,
                                            bool pin) {
...
template <typename Key_t, typename Value_t, typename Hash>
inline void LFUCache<Key_t, Value_t, Hash>::release(Handle_t handle) {
...
template <typename Key_t, typename Value_t, typename Hash>
inline void LFUCache<Key_t, Value_t, Hash>::pin(Handle_t handle) {
  ref(handle.node);
}

template <typename Key_t, typename Value_t, typename Hash>
inline typename LFUCache<Key_t, Value_t, Hash>::Handle_t
LFUCache<Key_t, Value_t, Hash>::preempt() {
...
template <typename Key_t, typename Value_t, typename Hash>
inline void LFUCache<Key_t, Value_t, Hash>::assign(Handle_t e) {
  ++capacity_;
  free_node(e.node);
}

template <typename Key_t, typename Value_t, typename Hash>
inline void LFUCache<Key_t, Value_t, Hash>::lookup_refresh(Node_t* node,
                                                           bool pin) {
  if (pin)
    ref(node);
  else if (node->refs == 1)
...
template <typename Key_t, typename Value_t, typename Hash>
inline typename LFUCache<Key_t, Value_t, Hash>::Handle_t
LFUCache<Key_t, Value_t, Hash>::refresh(Key_t key, uint32_t hash,
                                        Handle_t& successor) {
  // Disable support for capacity_ == 0; the user must set capacity first
  assert(capacity_ > 0);
...
template <typename Key_t, typename Value_t, typename Hash>
inline bool LFUCache<Key_t, Value_t, Hash>::erase(Handle_t handle) {
  Node_t* e = handle.node;
  assert(e);
...
template <typename Key_t, typename Value_t, typename Hash>
inline typename LFUCache<Key_t, Value_t, Hash>::Handle_t
LFUCache<Key_t, Value_t, Hash>::install(Key_t key) {
  return install_impl(key);
}

template <typename Key_t, typename Value_t, typename Hash>
inline typename LFUCache<Key_t, Value_t, Hash>::Node_t*
LFUCache<Key_t, Value_t, Hash>::install_impl(Key_t key) {
  Node_t* e;
...
template <typename Key_t, typename Value_t, typename Hash>
inline typename LFUCache<Key_t, Value_t, Hash>::Node_t*
LFUCache<Key_t, Value_t, Hash>::alloc_node() {
  ...
  // Evict one handle from LFU and recycle it
  if (lfu_.next == &lfu_) return nullptr;  // No more space
  Node_t* e = lfu_.next;
  assert(e->refs == 1);
  list_remove(e);  // Remove from lfu_
  [[maybe_unused]] Node_t* e_;
  e_ = table_->remove(e->key, e->hash);
  assert(e_ == e);
  --size_;
...
template <typename Key_t, typename Value_t, typename Hash>
inline void LFUCache<Key_t, Value_t, Hash>::free_node(Node_t* e) {
  list_append(&free_, e);
}

template <typename Key_t, typename Value_t, typename Hash>
inline void LFUCache<Key_t, Value_t, Hash>::ref(Node_t* e) {
  if (e->refs == 1) {  // If on lfu_ list, move to in_use_ list.
    list_remove(e);
    list_append(&in_use_, e);
  }
  e->refs++;
...
template <typename Key_t, typename Value_t, typename Hash>
inline void LFUCache<Key_t, Value_t, Hash>::list_remove(Node_t* e) {
  e->next->prev = e->prev;
  e->prev->next = e->next;
}

template <typename Key_t, typename Value_t, typename Hash>
inline void LFUCache<Key_t, Value_t, Hash>::list_append(Node_t* list,
                                                        Node_t* e) {
...
template <typename Key_t, typename Value_t, typename Hash>
inline std::ostream& LFUCache<Key_t, Value_t, Hash>::print(std::ostream& os,
                                                           int indent) const {
  os << "LFUCache (capacity=" << capacity_ << ") {\n";
